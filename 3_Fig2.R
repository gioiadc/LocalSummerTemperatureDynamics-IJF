# This code reproduce Figure 2 in the main manuscript 
...
for(site in 1:2){
  ...
  dataSeas[[site]] <- cbind(dati, site = location) 
  
  load(paste0("ExtractedResults/HatProbIn",location,"_up2018_Bayes.Rda"))
  load(paste0("ExtractedResults/Exc",location,"_up2018_Bayes.Rda"))
  Exc <- Exc[c(1,4,7,10)]
  ...
  for(K in 2:5){
    ...
    for(perc in c(1:2)){
      ...
      Concordance <- list()
      Concordance$Overall <- cbind(fileTemp$res[,c("years","doy", "MaxT", "tresh", "excedance")], Exc[[Kpos]]$HWs)
    ...
    ProbFore[[Kpos]] <- melt(HatProbIn[[Kpos]], id.vars = "doy")
    ProbFore[[Kpos]]$K <- K
  }
...
plotProb <- ggplot(bbb)+
  ...
  scale_fill_manual(values = c("#032163", "gold2", "darkorange2","orangered3","darkred"), guide = "none")+  
  theme_default()+
