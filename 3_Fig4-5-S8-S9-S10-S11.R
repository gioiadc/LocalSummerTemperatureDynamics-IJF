...
load("HeatwaveR/database_Hwr.Rdata")
...
load("ExtractedResults/TempTrajCapriva_from2019_to2024_Bayes_all.Rda")
load("ExtractedResults/StatesOutCapriva_from2019_to2024_Bayes.Rda")

outStates <- lapply(1:length(StatesOut), function(x){
  lapply(1:length(outTraj[[1]]), function(z) StatesOut[[x]][[z]][, idx])
})
...
foreHWdist_Capriva <- getForeHWdist2(trajectories = outTraj, 
                                     ...
                                     Nsim = 200,
                                     year_thresh = 2019) 
HWdist_CaprivaS <- do.call("rbind",lapply(1:length(outStates), function(x) do.call("rbind", foreHWdist_Capriva[[x]])))
...
foreHWdist_Capriva <- getForeHWdist(trajectories = outTraj, 
                                     ...
                                     Nsim = 200,
                                     year_thresh = 2019) 
HWdist_CaprivaT <- do.call("rbind",lapply(1:length(outStates), function(x) do.call("rbind", foreHWdist_Capriva[[x]])))
...
load("ExtractedResults/TempTrajTrieste_from2019_to2024_Bayes_all.Rda")
...
foreHWdist_Trieste <- getForeHWdist2(trajectories = outTraj, 
                                     ...
                                     Nsim = 200,
                                     year_thresh = 2019) 
HWdist_TriesteS <- do.call("rbind",lapply(1:length(outStates), function(x) do.call("rbind", foreHWdist_Trieste[[x]])))
...
foreHWdist_Trieste <- getForeHWdist(trajectories = outTraj, 
                                    ...
                                    Nsim = 200,
                                    year_thresh = 2019) 
HWdist_TriesteT <- do.call("rbind",lapply(1:length(outStates), function(x) do.call("rbind", foreHWdist_Trieste[[x]])))
...
p1 <- ggplot(HWdist_all_v2[HWdist_all_v2$site == "Capriva"&HWdist_all_v2$measure == "HWd"&HWdist_all_v2$K!="HWr"
  ...
  geom_boxplot(col = "gray20", outlier.size = .5, alpha = 0.6) +
  ...
  scale_fill_manual(
    name = "K-th regime distribution",
    values = c(
      "Normal" = "#A3A3A3", "Gumbel" = "#E2122B", "rWeibull" = "#032163")
  )+
...
p1 <- ggplot(HWdist_all_v2[HWdist_all_v2$site == "Trieste"&HWdist_all_v2$measure == "HWd"&HWdist_all_v2$K!="HWr"
                           &HWdist_all_v2$method == "State",], 
             aes(x = as.factor(K), y = Freq, fill = last.regime)) +
  geom_boxplot(col = "gray20", outlier.size = .5, alpha = 0.7) +
