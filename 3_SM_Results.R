...
# - the posterior parameter summaries and trace plots for Capriva
...
# Posterior parameter summaries for Capriva - K = 5 ----

for(i in 1:3){
  fileName <- list.files(path = "ResultsGST/", pattern = paste0("fit_NNNN", last.regime[i], "_C_s.RData"))
  modelName <- gsub("_s.RData", '', fileName)
  
  load(paste0("ResultsGST/",fileName))
  fit <- mget(modelName, envir = globalenv())[[1]]
  rm(list=grep(modelName,ls(),value=TRUE))
  
  
  if(last.regime[[i]] == "N" | last.regime[i] == "G"){
    TABLE[[i]] <- xtable(round(summary(fit, pars=c("betaL1_new","betaL2_new","betaL3_new", "betaL4_new", "betaL5_new",
...
save(TABLE, file = "ExtractedResults/TableCoefficients19952018.Rdata")

# The results in the corresponds to 
#TABLE[[1]]  

# Trace plots for Capriva - K = 5 ----

fileName <- list.files(path = "ResultsGST/", pattern = "fit_NNNNN_C_s.RData")
log_lik <- list()
loo <- list()
div <- list()
modelName <- gsub("_s.RData", '', fileName)
...
xtable(round(summary(fit, pars=c("betaL1_new","betaL2_new","betaL3_new", "betaL4_new", "betaL5_new",
                                 "betaS1_new","betaS2_new","betaS3_new", "betaS4_new", "betaS5_new",
                                 "betaTp01_new", "betaTp10_new",
                                 "betaTp12_new", "betaTp21_new",
                                 "betaTp23_new", "betaTp32_new",
...
R1.1 <- mcmc_trace(fit, regex_pars = c("betaL1_new", "betaS1_new"),
                   facet_args = list(ncol = 1, labeller = as_labeller(labels[1:7], label_parsed)))+guides(colour="none") 
R1.2 <- mcmc_trace(fit, regex_pars = c( "ltauL1","betaTp01_new", "betaTp10_new", "betaTp12_new",
                                        "betaTp21_new", "betaTp23_new", "betaTp32_new", "betaTp34_new", "betaTp43_new"),
                   facet_args = list(ncol = 1, labeller = as_labeller(labels[8:16], label_parsed)))+guides(colour="none") 

R1acf.1 <- mcmc_acf(as.matrix(fit, pars=c("betaL1_new", "betaS1_new")),
                    facet_args = list(ncol = 1, labeller = as_labeller(labels[1:7], label_parsed)))+guides(colour="none") 
R1acf.2 <- mcmc_acf(as.matrix(fit, pars=c("ltauL1","betaTp01_new", "betaTp10_new", "betaTp12_new",
                                          "betaTp21_new", "betaTp23_new", "betaTp32_new","betaTp34_new", "betaTp43_new")),
                    facet_args = list(ncol = 1, labeller = as_labeller(labels[8:16], label_parsed)))+guides(colour="none") 
...
X_list <- list(
  ...
  do.call("rbind", TempTraj[[7]]),
  do.call("rbind", TempTraj[[10]])
)
...
# Maximum temperature plot for 2023 and 2024 years with the empirical based
# threshold ----
